...
class EnhancedSimulator:
    def __init__(self, xml_path, end_joint, robot_model_path, cam_res=(1280, 1280)):
        ...
        self.n_substeps = 10  # [V6] 提高物理更新频率以获得更平滑的效果

        # [V6 核心] 状态机与动力学参数
        self.scan_wait_time = 0.5  # 到达后停留的时间 (秒)
        self.arrival_start_time = None  # 记录到达目标的时间点
        self.is_stabilized = False  # 标记是否处于稳定驻留状态

        # [V6] 动力学限制参数
        # 加速度设为 12.0，保证从远处飞回来时反应够快
        self.joint_vel_limit = 3.0  # rad/s
        self.joint_acc_limit = 12.0  # rad/s^2

        # 状态记忆 (用于二阶微分平滑)
        self.last_cmd_qpos = None
        self.last_cmd_qvel = None

        # 目标与控制
        self.target_qpos_ik = None  # 纯 IK 算出的理想目标
        self.ctrl_qpos = None  # 经过限制器后的实际下发指令
    ...
    def init_mujoco(self):
        try:
            ...
            self.target_qpos_ik = init_q
            self.ctrl_qpos = init_q
            self.last_cmd_qpos = init_q
            self.last_cmd_qvel = np.zeros(6)

            self.data.ctrl[:6] = init_q
    ...
    def compute_target_matrix(self, position, normal, standoff=0.1):
        ...
        # 2. [核心修复] 锁定辅助轴 (World X)
        # 只要 Z 轴不与世界 X 轴平行，就强行让相机“上方”尽量朝向世界 X
        global_ref = np.array([1, 0, 0])
        # 奇异点保护
        if abs(np.dot(z_axis, global_ref)) > 0.95:
            global_ref = np.array([0, 1, 0])

        # 3. 计算 Y (Z x Ref) -> 相机右
        y_axis = np.cross(z_axis, global_ref)
    ...
    def apply_dynamics_limits(self, target_q, dt):
        """
        [V6] 动力学平滑器
        负责 "Move" 阶段的物理平滑，模拟 ServoJ 接口。
        """
        if self.last_cmd_qpos is None:
            self.last_cmd_qpos = self.data.qpos[:6].copy()
            self.last_cmd_qvel = np.zeros(6)

        dist = target_q - self.last_cmd_qpos

        # [安全机制] IK 跳变保护
        if np.max(np.abs(dist)) > 1.5:
            # print("[Warning] IK Jump Detected! Resetting.")
            self.last_cmd_qpos = target_q
            self.last_cmd_qvel = np.zeros(6)
            return target_q

        # [物理核心] 平方根刹车律
        # 根据剩余距离计算最大安全速度，防止超调震荡
        max_v_safe = np.sqrt(2 * self.joint_acc_limit * np.abs(dist))

        # 梯形速度规划
        v_target = np.sign(dist) * np.minimum(self.joint_vel_limit, max_v_safe)

        # 加速度限制
        a_needed = (v_target - self.last_cmd_qvel) / dt
        a_applied = np.clip(a_needed, -self.joint_acc_limit, self.joint_acc_limit)

        # 二阶积分
        new_vel = self.last_cmd_qvel + a_applied * dt
        new_pos = self.last_cmd_qpos + new_vel * dt

        self.last_cmd_qvel = new_vel
        self.last_cmd_qpos = new_pos
        return new_pos
    ...
    def step(self):
        ...
        if not self.paused:
            ...
            if self.path_points is not None and len(self.path_points) > 0:
                ...
                # A. 始终解算当前目标点的 IK
                current_pt = self.path_points[self.current_idx]
                if self.path_normals is not None:
                    current_nm = self.path_normals[self.current_idx]
                else:
                    current_nm = np.array([0, 0, 1])

                self.perform_ik_step(current_pt, current_nm)

                # B. 检测是否到位 & 处理驻留计时
                current_q = self.data.qpos[:6]
                if self.target_qpos_ik is not None:
                    # 计算误差 (L_inf norm)
                    error = np.max(np.abs(current_q - self.target_qpos_ik))

                    # [V6 核心优化]：消除标定卡顿
                    # 如果是标定模式下的中间点，我们允许更大的误差通过，并且不需要停车等待
                    is_calib_intermediate = self.calibration_mode and (self.current_idx < len(self.path_points) - 1)

                    # 标定中间点阈值放宽到 0.05，且等待时间为 0
                    tolerance = 0.05 if is_calib_intermediate else 0.02
                    ...
                    if error < tolerance:
                        ...
                        # 如果是标定中间点，wait_time 强制为 0 (立即过)
                        effective_wait_time = 0.0 if is_calib_intermediate else self.scan_wait_time

                        elapsed = time.time() - self.arrival_start_time
                        if elapsed > effective_wait_time:
                            # === 切换下一个点 ===
                            if self.calibration_mode:
                                if self.current_idx < len(self.path_points) - 1:
                                    self.current_idx += 1
                                    self.arrival_start_time = None
                                    self.is_stabilized = False  # 中间过程认为不稳定，只有最后停下才稳
                            else:
                                # 正常扫描循环
            ...
            # --- 物理仿真执行 (Move Phase) ---
            # 无论是在移动还是在驻留，都通过 apply_dynamics_limits 发送指令
            # 驻留时，target 不变，dynamics limits 会自动将速度降为 0 并利用伺服力矩保持位置
            for _ in range(self.n_substeps):
                if self.target_qpos_ik is not None:
                    smooth_ctrl = self.apply_dynamics_limits(self.target_qpos_ik, self.physics_dt)
                    self.ctrl_qpos = smooth_ctrl
                    self.data.ctrl[:6] = smooth_ctrl
    ...
    def perform_ik_step(self, pos=None, normal=None):
        if self.path_points is None: return

        # 兼容旧接口调用 (无参数时使用 current_idx)
        if pos is None:
            pos = self.path_points[self.current_idx]
            normal = self.path_normals[self.current_idx] if self.path_normals is not None else np.array([0, 0, 1])

        target_pt = pos
        # [V6] 标定点已统一为 3D 坐标，不需要处理 6D 逻辑了
        T_target = self.compute_target_matrix(pos, normal, self.scan_height)
        ...
        try:
            ...
            if info["success"]:
                # 更新 raw target
                q_raw = np.clip(q_sol, self.model.jnt_range[:6, 0], self.model.jnt_range[:6, 1])
                self.target_qpos_ik = q_raw
            else:
                if self.calibration_mode:
                    pass
        except Exception as e:
    ...
    def start_calibration(self):
        ...
        for i in range(steps):
            ...
            path.append(pos)
            # 标定模式法线向下 (Z轴正方向在 MuJoCo 中通常对应 world 的 Z)
            normals.append(np.array([0, 0, 1]))

        self.path_points = np.array(path)
        self.path_normals = np.array(normals)
        self.current_idx = 0
        self.arrival_start_time = None
    ...
    def set_path(self, points, normals, height=0.1):
        ...
        self.current_idx = 0
        self.arrival_start_time = None  # 重置计时器

        self.paused = True

        # [V6 核心修复] 移除瞬移代码 (self.data.qpos[:] = ...)
        # 计算起点的 IK
        if len(points) > 0:
            self.perform_ik_step(points[0], normals[0])

            # 重点：将平滑器的"上一帧位置"重置为"当前真实位置"
            # 这样平滑器就会规划一条从"当前位置"到"起点"的轨迹
            self.last_cmd_qpos = self.data.qpos[:6].copy()
            self.last_cmd_qvel = np.zeros(6)
    ...
    def manual_adjust(self, direction):
        ...
        self.arrival_start_time = None  # 重置计时

        pt = self.path_points[self.current_idx]
        nm = self.path_normals[self.current_idx] if self.path_normals is not None else None

        self.perform_ik_step(pt, nm)
        if self.target_qpos_ik is not None:
            # 手动调节时保留瞬移，方便调试
            self.data.qpos[:6] = self.target_qpos_ik
            self.last_cmd_qpos = self.target_qpos_ik.copy()
    ...
    def _add_markers(self, scene):
        ...
        if self.path_points is not None and len(self.path_points) > 0:
            # 当前点 (根据状态变色)
            # [V6] 绿色=稳定驻留 (Scanning), 红色=移动中 (Moving)
            color = [0, 1, 0, 1] if self.is_stabilized else [1, 0, 0, 1]
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Robot Scan V6 Full (Smooth Calibration & Realistic Path)")
    ...
    def create_settings_panel(self):
        ...
        self.inp_interval.setPlaceholderText("ms (Stop&Scan)")

        self.inp_height = QLineEdit("0.1")

        # [V6] 动力学参数 (Move 阶段)
        self.inp_max_vel = QLineEdit("3.0")
        self.inp_max_vel.setPlaceholderText("Max Vel (rad/s)")
        self.inp_max_acc = QLineEdit("12.0")  # V6: 提高默认加速度
        self.inp_max_acc.setPlaceholderText("Max Acc (rad/s^2)")

        f.addRow("扫描步长(mm):", self.inp_step)
        f.addRow("驻留时间(ms):", self.inp_interval)
        f.addRow("法向Z阈值:", self.inp_z_thresh)
        f.addRow("最大半径(mm):", self.inp_radius)
        f.addRow("扫描高度(m):", self.inp_height)

        f.addRow("Max Joint Vel:", self.inp_max_vel)
        f.addRow("Max Joint Acc:", self.inp_max_acc)
    ...
    def generate_and_send(self):
        ...
        if self.sim:
            try:
                ...
                self.sim.scan_wait_time = ms / 1000.0

                self.sim.joint_vel_limit = self.get_float(self.inp_max_vel, 3.0)
                self.sim.joint_acc_limit = self.get_float(self.inp_max_acc, 12.0)
                print(
                    f"Updated: Wait={self.sim.scan_wait_time}s, Vel={self.sim.joint_vel_limit}, Acc={self.sim.joint_acc_limit}")
            except:
                pass
    ...
    def game_loop(self):
        ...
        try:
            ...
            # [V6] 显示状态：Moving vs Scanning
            status_text = "SCANNING" if self.sim.is_stabilized else "MOVING"
            self.lbl_status.setText(f"FPS:{self.current_fps} | {status_text} | {mode}")
